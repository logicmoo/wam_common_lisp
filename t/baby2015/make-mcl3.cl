           ...
           (make-instance 'button-dialog-item
             ...
             #'(lambda (item)
                 (let ((dialog (view-container item)))
                   (eval-enqueue
                      `(progn
                         (window-hide *babylon-configure-dialog*)
                         ...
                         (loop 
                           (if (y-or-n-p "Is your AntiVirus software temporarily disabled?") 
                             (return)))
                         (format t "~%Use Save Application... from the Tools menu to create your Babylon image now")
                         )))))
           ))))
...
(progn
  (push (symbol-function 'deallocate-babylon-configure-dialog) 
        *save-exit-functions*)
  (setf *lisp-startup-functions*                                     ;;; *restore-lisp-functions*
        (append *lisp-startup-functions*                             ;;; *restore-lisp-functions*
                (list (symbol-function 'babylon-default-menubar)
                      (symbol-function 'babylon-initialize-image))))
  (babylon-default-menubar)
  (allocate-babylon-configure-dialog)
  (window-select *babylon-configure-dialog*)
  )
