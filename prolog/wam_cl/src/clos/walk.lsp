...
(in-package 'walker)
(proclaim '(notinline note-lexical-binding walk-bindings-1 walk-let/let*
	    walk-form-internal))
;;;
...
(defun note-declaration (declaration env)
  (let ((lock (env-lock env)))
    (setf (third lock)
	  (cons declaration (third lock)))))

(defun note-lexical-binding (thing env)
  (let ((lock (env-lock env)))
    (setf (fourth lock)
	  (cons thing (fourth lock)))))
