...
(defun f (x) (labels ((g () (funcall x))) (g)))
(defun f1 (x) (labels ((g () (funcall g x))) (g)))
(defun f2 (x) (labels ((g (y) (funcall y x))) (g 0)))

#|
#+:CCL (rplaca (symbol-function 'foo) 'lambda-block)

(defun d () (disassemble1 'foo))
(defmacro defn (&rest body)
  #+:CCL `(rplacd (symbol-function 'foo) ',body)
  #+KCL `(defun ,@body))
(defun test ()
  (compile-file "test.lsp" :c-file t :h-file t :data-file t :o-file nil))
|#
...
;;; cmpblock.lsp
(defun t68 () (flet ((bar () (return-from t65))) (bar)))
(defun t69 () (flet ((bar () (return-from t66))) #'bar))
(defun t70 () (block b (bar #'(lambda (x) (return-from b)))))
