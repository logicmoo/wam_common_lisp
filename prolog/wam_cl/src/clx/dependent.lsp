...
;;;(c) Copyright Enhancements by Giuseppe Attardi, 1994.
;;;
...
#-(or ecl excl lcl3.0 clx-overlapping-arrays)
(progn
...
#+ecl
(declaim (ftype (function (character) card8) char->card8)
	 (ftype (function (card8) character) card8->char))
	       ...
	       (cond ((dolist (pair alist nil)
			(when (not (= (char-code (car pair)) (cdr pair)))
			 ...
			 (progn
			     ...
			     #.(declare-buffun)
			     (the card8 (aref (the #+ecl buffer-bytes
						   #-ecl (simple-array card8 (*))
...
#-(or explorer Genera lucid kcl ibcl excl Minima ecl)
...
(defun open-x-stream (host display protocol)
  (declare (ignore protocol))
  (let ((stream (si:open-client-stream host (+ *x-tcp-port* display))))
    (if (streamp stream)
	stream
      (error "Failed to connect to server: ~A:~D" host display))))
...
(defun buffer-read-default (display vector start end timeout)
  ...
  (let ((stream (display-input-stream display)))
    ...
    (or (cond ((null stream))
	      ...
	      ((eql timeout 0) :timeout)
	      ((buffer-input-wait-default display timeout)))
	(minusp (si:read-bytes stream vector start end)))))
...
#-(or Genera explorer excl lcl3.0 Minima ecl)
(defun buffer-read-default (display vector start end timeout)
  (declare (type display display)
	   (type buffer-bytes vector)
	   (type array-index start end)
...
(defun buffer-write-default (vector display start end)
  (declare (type buffer-bytes vector)
	   ...
	   (type array-index start end))
  (declare-buffun)
  (si:write-bytes (display-output-stream display) vector start end))
  ...
  (flet ((reporter-for-condition (name)
	...
	(setq report-function (reporter-for-condition (first parent-types))))
	 ...
	 (defstruct (,name (:conc-name ,(intern conc-name))
		     ...
		     (:copier nil)	; save space. ECL.
		     (:predicate nil))	; idem. ECL
	   ,@slot-names)
	 ,(when documentation
	    `(setf (documentation name 'type) ,documentation))
	 ,(when reporter
...
#-(or lispm clx-ansi-common-lisp excl lcl3.0 clos)
...
#+clos
(defmethod print-object ((condition x-error) stream)
  (declare (type stream stream))
  (if *print-escape*
      (format stream "#<CONDITION ~a>" (type-of condition))
    (funcall (x-error-report-function condition) condition stream))
  condition)
