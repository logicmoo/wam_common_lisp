...
(deftype card32 () '(integer 0 4294967295)) ; 2^32 - 1

(defun card29p (x)
  (declare (fixnum x))
  (and (>= x 0) (<= x 536870911)))
(deftype card29 () '(and fixnum (satisfies card29p)))

(deftype int32 () 'fixnum)

(defun card16p (x)
  (declare (fixnum x))
  (and (>= x 0) (<= x 65535)))
(deftype card16 () '(and fixnum (satisfies card16p)))

(defun int16p (x)
  (declare (fixnum x))
  (and (>= x -32768) (<= x 32767)))
(deftype int16 () '(and fixnum (satisfies int16p)))

(defun card8p (x)
  (declare (fixnum x))
  (and (>= x 0) (<= x 255)))
(deftype card8 () '(and fixnum (satisfies card8p)))
...
#+ecl
(deftype pixel () '(integer 0 4294967295)) ; 2^32 - 1
#-ecl
