;;; Received from: Bill.Chiles@WB1.CS.CMU.EDU
...
(in-package "DEMOS" :use '("LISP" "XLIB"))
...
(defvar *host* (machine-instance))	; set this properly
...
;;; Use this for color displays:
(eval-when (load compile eval) (pushnew 'color *features*))

(defvar *colormap*)
(defvar *colors* (make-array 10 :element-type 'stringable
                             :initial-contents '("white" "black" "red" "green"
                                                 "blue" "yellow" "orange" 
						 "brown" "gray" "violet")))

(defvar *color-pixels* (make-array 10 :element-type 'pixel))

(defun create-colors ()
  (let* ((colormap (xlib:screen-default-colormap *screen*)))
    (dotimes (i (length *color-pixels*))
      (setf (aref *color-pixels* i)
	    (xlib:alloc-color colormap (aref *colors* i))))
    (values colormap)))

;;; ----------------------------------------------------------------------

(defmacro defdemo (fun-name demo-name args x y width height background doc &rest forms)
  `(progn
     (defun ,fun-name ,args
       ,doc
       (cond (*display*
...
(defun greynetic (window duration background foreground)
  (let* ((pixmap (xlib:create-pixmap :width 32 :height 32
...
(defdemo greynetic-demo "Greynetic" (&optional (duration 300))
  100 100 600 600 (aref *color-pixels* 2)
  "Displays random grey rectangles."
  (greynetic *window* duration
	     (aref *color-pixels* 8) (aref *color-pixels* 3)))
...
(defstruct qix
  buffer
  (dx1 5 :type fixnum)			; Beppe
  (dy1 10 :type fixnum)			; Beppe
  (dx2 10 :type fixnum)			; Beppe
  (dy2 5 :type fixnum))			; Beppe
...
(defun qix (window lengths duration background foreground)
  ...
  (let ((histories (mapcar #'construct-qix lengths)))
	...
	(let ((x (random width))
      ...
      (let ((x1 0) (y1 0) (x2 0) (y2 0) (dx1 0) (dy1 0) (dx2 0) (dy2 0)
	    (tem 0) line next-line qix
	    (gc (xlib:create-gcontext :drawable window
				      :foreground foreground
				      :background background
				      :line-width 0 :line-style :solid
				      :function boole-c2)))
	(declare (fixnum x1 y1 x2 y2 dx1 dy1 dx2 dy2 tem)) ; Beppe
	(declare (type (vector fixnum) line next-line)) ; Beppe
	(dotimes (i duration)
	  ...
	  (setq dx1 (- (the fixnum (+ (qix-dx1 qix) (random 3))) 1)) ; Beppe
	  (setq dy1 (- (the fixnum (+ (qix-dy1 qix) (random 3))) 1)) ; Beppe
	  (setq dx2 (- (the fixnum (+ (qix-dx2 qix) (random 3))) 1)) ; Beppe
	  (setq dy2 (- (the fixnum (+ (qix-dy2 qix) (random 3))) 1)) ; Beppe
	  ...
	  (setf (svref next-line 0) (the fixnum (+ x1 dx1))) ; Beppe
	  (setf (svref next-line 1) (the fixnum (+ y1 dy1))) ; Beppe
	  (setf (svref next-line 2) (the fixnum (+ x2 dx2))) ; Beppe
	  (setf (svref next-line 3) (the fixnum (+ y2 dy2))) ; Beppe
	  (xlib:display-force-output *display*))))))


(defdemo qix-demo "Qix" (&optional (lengths '(30 30)) (duration 2000))
  500 0 300 300 (aref *color-pixels* 4)
  "Hypnotic wandering lines."
  (qix *window* lengths duration
       (aref *color-pixels* 5) (aref *color-pixels* 4)))
...
(proclaim '(type (vector fixnum) sin-array)) ; Beppe
...
(defmacro psin (val)
  `(let* ((val ,val)
	  ...
	  (frac 0)
	  (sinlo 0))
    (declare (fixnum val frac sinlo))	; Beppe
     (if (>= val d180)
	 (setq neg t
	       val (- val d180)))
     (if (>= val d90)
	 (setq val (- d180 val)))
     (setq frac (the fixnum (logand val 7))) ; Beppe
     (setq val (ash val -3))
     ;; 
...
(defmacro high-16bits-* (a b)
  `(let ((a-h (ash ,a -8))
	 (b-h (ash ,b -8)))
    (declare (fixnum a-h b-h))		; Beppe
    (+ (* a-h b-h)
     (ash (* a-h (the fixnum (logand ,b 255))) -8) ; Beppe
     (ash (* b-h (the fixnum (logand ,a 255))) -8)))) ; Beppe

(defun complete (style petal)
  (declare (fixnum style petal))
  (let ((repnum 1)
	(factor 0) (cntval 0) (needed 0))
    (declare (fixnum repnum factor cntval needed)) ; Beppe
    (dotimes (i 3)
      (case i
...
(defun petal (petal-window background foreground
			   &optional (how-many 10) (style 0) (petal 0))
  (let ((width 512)
	(height 512))
    (declare (fixnum width height))	; Beppe
    ...
    (let ((veccnt 0)
	  ...
	  (gc (xlib:create-gcontext :drawable petal-window
				    ...
				    :background background
				    :line-width 0 :line-style :solid)))
      (declare (fixnum veccnt nustyle nupetal scalfac ctrx ctry tt s
		       lststyle lstpetal petstyle vectors r x1 y1 x2
		       y2 i)) ; Beppe
      (loop
	(when (zerop veccnt)
...
(defdemo petal-demo "Petal" (&optional (how-many 10) (style 0) (petal 0))
  100 100 512 512 (aref *color-pixels* 3)
  "Flower-like display."
  (petal *window* (aref *color-pixels* 3) (aref *color-pixels* 4)
	 how-many style petal))
...
(defun hanoi (window n background foreground)
  (multiple-value-bind (width height) (full-window-state window)
...
(defdemo slow-hanoi-demo "Slow-towers-of-Hanoi" (&optional (how-many 4))
  0 100 768 300 (aref *color-pixels* 9)
  "Solves the Towers of Hanoi problem before your very eyes."
  (let ((*horizontal-velocity* 3)
	(*vertical-velocity* 1))
    (hanoi *window* how-many (aref *color-pixels* 2) (aref *color-pixels* 9))))
;;;
...
(defun recurrence (display window background foreground
...
(defdemo recurrence-demo "Recurrence" ()
  10 10 700 700 (aref *color-pixels* 2)
  "Plots a cool recurrence relation."
  (recurrence *display* *window*
	      (aref *color-pixels* 4) (aref *color-pixels* 2)))
...
(defun plaid (display window background foreground
		      &optional (num-iterations 10000) (num-rectangles 10))
  (let ((gcontext (xlib:create-gcontext :drawable window
					:function boole-c2
					#-COLOR :plane-mask
					#-COLOR (logxor *white-pixel*
							*black-pixel*)
					:background background
					:foreground foreground
					:fill-style :solid))
...
(defdemo plaid-demo "Plaid" (&optional (iterations 10000) (num-rectangles 10))
  10 10 101 201 (aref *color-pixels* 3)
  "Plaid, man."
  (plaid *display* *window*
	 (aref *color-pixels* 3) (aref *color-pixels* 8)
	 iterations num-rectangles))
...
(defvar *ball-size-x* #-COLOR 38 #+COLOR 36)
...
(defstruct (ball (:copier nil) (:predicate nil))
  (x (random (- *max-bball-x* *ball-size-x*)) :type fixnum) ; Beppe
  (y (random (- *max-bball-y* *ball-size-y*)) :type fixnum) ; Beppe
  (dx (if (zerop (random 2)) (random maximum-x-drift)
	  (- (random maximum-x-drift))) :type fixnum) ; Beppe
  (dy 0 :type fixnum))			; Beppe
...
(defun bounce-1-ball (pixmap window gcontext ball)
  (let ((x (ball-x ball))
    ...
    #+COLOR
    (xlib:clear-area window :x x :y y :width 36 :height 34) 
    (setq x (+ x dx))
    (setq y (+ y dy))
...
(defun bounce-balls (display window background foreground how-many duration)
  (declare (fixnum how-many))		; Beppe
  (xlib:clear-area window)
  (xlib:display-force-output display)
  (multiple-value-bind (*max-bball-x* *max-bball-y*) (full-window-state window)
    (let* ((balls (do ((i 0 (1+ i))
	   ...
	   (gcontext (xlib:create-gcontext :drawable window
					   ...
					   #+COLOR boole-1
					   :exposures :off))
	   (bounce-pixmap (xlib:create-pixmap :width 38 :height 34
					      :depth (xlib:drawable-depth window)
					      :drawable window))
...
(defdemo bouncing-ball-demo "Bouncing-Ball" (&optional (how-many 5) (duration 500))
  34 34 700 500 (aref *color-pixels* 3)
  "Bouncing balls in space."
  (bounce-balls *display*  *window*
		(aref *color-pixels* 3) (aref *color-pixels* 2)
		how-many duration))
