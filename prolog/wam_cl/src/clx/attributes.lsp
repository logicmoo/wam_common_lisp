...
(defun change-window-attribute (window number value)
  ...
  (let ((state-entry nil)
	(changes nil))
    #+ecl (declare (gcontext-state changes))
...
(defun put-drawable-geometry-changes (window changes)
  ...
  (let* ((display (window-display window))
	...
	(do ((bits mask (ash bits -1))
	     ...
	     (i 1 (index+ i 1)))		;Entry count
	    ((zerop bits)
	     (card16-put 2 (index-incf request-size)) ; ECL
	     (index-incf (buffer-boffset display)
			 (index* request-size 4))) ; ECL
	  (declare (type mask16 bits)
		   (type array-index request-size) ; ECL
		   (type array-index i))
	  (when (oddp bits)
	    (card29-put (index* (incf request-size) 4) ; ECL
			(aref changes i))))))))
