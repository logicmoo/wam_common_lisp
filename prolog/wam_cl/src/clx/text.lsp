...
(defun translate-default (src src-start src-end font dst dst-start)
  ...
  (if (stringp src)
	...
	(if (graphic-char-p (setq char (char src i)))
	    #-ecl (setf (aref dst j) (char->card8 char))
	    #+ecl (if (eq (type-of dst) 'STRING)
		      (setf (schar dst j) (int-char (char->card8 char)))
	...
	(cond ((and (characterp elt) (graphic-char-p elt))
	       #-ecl (setf (aref dst j) (char->card8 elt))
	       #+ecl (if (eq (type-of dst) 'STRING)
			 (setf (schar dst j) (int-char (char->card8 elt)))
	...
	(with-buffer-flush-inhibited (display)
	  (do* ((boffset (index+ buffer-boffset 16))
		...
		(do ((end (the array-index (lround boffset))))
		    ((index>= boffset end))
		  #-ecl (setf (aref buffer-bbuf boffset) 0)
		  #+ecl (setf (aref-card8 buffer-bbuf boffset) 0)
		  (index-incf boffset))
		...
		#-ecl (setf (aref buffer-bbuf boffset) dst-chunk)
		#-ecl (setf (aref buffer-bbuf (index+ boffset 1)) offset)
		#-ecl (incf boffset (index+ dst-chunk 2))
		#+ecl (setf (aref-card8 buffer-bbuf boffset) dst-chunk)
		#+ecl (setf (aref-card8 buffer-bbuf (index+ boffset 1)) offset)
		#+ecl (index-incf boffset (index+ dst-chunk 2)))
	      (setq offset 0)
	      (cond ((null new-font)
