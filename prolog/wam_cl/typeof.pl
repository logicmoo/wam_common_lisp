...
cl_class_of(Obj,ugly(claz_builtin_class,Type)):- type_i(Obj,Type).
cl_class_of(function(OP),Class):- get_opv(OP,function,Obj),!,cl_class_of(Obj,Class).
cl_class_of(Obj,ugly(claz_class,Type)):- type_ges(Obj,Type).
cl_class_of(_,ugly(claz_builtin_class,t)).

type_i(Var,sys_locative):-var(Var).
type_i([],null):-!.
type_i([_|_],cons):-!.
type_i(ugly(Type,_Data),Type).
...
type_named(ugly(_,Type),Type):- atom(Type),!.
