...
file_sexpr_with_comments(C)                 --> sexpr_dcgPeek(`#|`),!,zalwayz(comment_expr(C)),swhite,!.
file_sexpr_with_comments(C)                 --> sexpr_dcgPeek(`;`),!, zalwayz(comment_expr(C)),swhite,!.
...
zalwayz(G,H,T):- nb_current('$lisp_translation_stream',S),is_stream(S), \+ stream_property(S,tty(true)),always_b(G,H,T).
